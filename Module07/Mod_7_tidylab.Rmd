...
fancyRpartPlot(tree.carseats, caption = NULL)
```

See the importance of each predictor in from rpart
```{r}
tree.carseats$variable.importance
```
We see **price** and **self location** are most important.

```{r}
printcp(tree.carseats)
```
Above are the 10-fold cross-validation results for each level of splitting.

The **rel error** of each iteration of the tree is the fraction of mislabeled elements in the iteration relative to the fraction of mislabeled elements in the root. In this example, 41% of training cases are fraudulent. so after the first split .41 * .71 % of the responses would be mislabled.

As a rule of thumb, it’s best to prune a decision tree using the cp of smallest tree that is within one standard deviation of the tree with the smallest xerror. In this example, the best xerror is 0.61585 with standard deviation 0.052981. 

```{r}
0.61585 + 0.052981
```

So, we want the smallest tree with xerror less than 0.668831. This is the tree with cp = 0.030488, so we’ll want to prune our tree with a cp slightly greater than than 0.0304 == 0.031

```{r}
pruned.carseats.tree <- prune(tree.carseats, cp = 0.031)
fancyRpartPlot(pruned.carseats.tree)
```
...
carseats.tree_fits <- caret::train(High~.-Sales, data=carseats, 
                      ...
                      trControl=caret::trainControl("repeatedcv", number=5,
                                                    returnResamp='all',
