...
```{r q10i-1}
weekly.mod.log.train = glm(Direction~Lag2+I(Volu), data=Weekly.train,family=binomial)
#weekly.mod.log.train %>% tidy()
weekly.mod.log.train.logOdds_predictions = weekly.mod.log.train %>% augment(newdata=Weekly.test) %>% 
  dplyr::select(Direction, .fitted)  %>% 
  mutate(.prediction=ifelse(.fitted<0, "Down", "Up")) %>%
...
**Answer q11b**: Variables that appear strongly correlated with mpg01 are: 
...
Auto.lda.pred.augmented = Auto.lda.pred %>% add_column(Y) 
Auto.lda.pred.augmented %>% conf_mat(mpg01, prediction)
```
**LDA Sensitivity:** 51 / 51 = **1.0**
<br /><br />
**LDA Specificity:** 41 / 49 = **0.84**
<br /><br />
**Total Accuracy:** 92 / 100 = 0.92
<br /><br />
...
Auto.qda.pred.augmented = Auto.qda.pred %>% add_column(Y) 
Auto.qda.pred.augmented %>% conf_mat(mpg01, prediction)
```
...
outcome
```
**Answer q11g**: My ***best** value of k was 24 when I compare sensitivity, specificity, and balanced accuracy. With a k of 24:
<br />
**KNN | k=24 Sensitivity 0.96**
<br /><br />
**KNN | k=24 Specificity: 0.837**
<br /><br />
