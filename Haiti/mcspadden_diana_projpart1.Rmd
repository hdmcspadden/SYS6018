...
For logistic regression, LDA, **TO ADD....TO ADD** Cross-Validation threshold performance used ROC for tuning.
...
<br /><br />
Here I run without any cross-validation on the entire train dataset just to take a look at performance

``` {r warning=FALSE}
# train logistic regression on imbalanced data
log.model<- glm(ClassBinary ~ Blue+Green+Red+GBSqr+RBSqr, data=train, family=binomial())

confusion_matrix(log.model)
```
...
<ul>
<li>TPR: 96.7 / 96.8 = 0.999</li>
<li>FPR: 0.3 / 3.1 = 0.097</li>
<li>Precision: 96.7 / 97 = 0.997</li>
</ul>


### Logistic Regression Training ROC Curve:
...
**The ROC-AUC for the 10-fold cross-validated training data is: 1.0.**
...
### Logistic Regression Testing ROC
```{r logROCTest}
#plot(roc.curve(test.YesNo$ClassBinary, log.test$ClassBinary))
...
<tr>
  ...
  <td><!-- Log Reg TPR -->0.999</td>
  <td><!-- Log Reg FPR -->0.097</td>
...
<tr>
  ...
  <td><!-- Log Reg AUROC -->1.0</td>
  <td><!-- Log Reg Threshold -->1.0</td>
  <td><!-- Log Reg Accuracy -->0.9955</td>
  <td><!-- Log Reg TPR -->0.998</td>
  <td><!-- Log Reg FPR -->0.088</td>
  <td><!-- Log Reg Precision -->0.997</td>
</tr>
