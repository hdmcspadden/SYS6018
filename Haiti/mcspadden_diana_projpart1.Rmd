...
For logistic regression, LDA, QDA, KNN  and Penalized Logistic Regression Cross-Validation threshold performance use ROC for tuning.
...
$$ClassBinary = Blue + GBSqr + RBSqr \hspace{.3cm} | \hspace{.3cm} GBSqr = (Green + Blue)^2 \hspace{.3cm} and \hspace{.3cm} (Red + Blue)^2$$
...
```
**Model 3**<br />
Only p values of .1, .2, .3 and .35 was Model 3, the simple model, was unable to differentiate between blue tarps and not blue tarps. The sensitivity, specificity and precision did not approach the performance of the other two models. Model 3 will no longer be considered.
<br />
**Model 1 and Model 2**<br />
The differences between Model 1, the more complex mode, and Model 2, the additive model, are more nuanced.
<ul>
...
<br />
Model 3 performed better on all measures.
<br />
Because geographic distribution of blue tarps is not included in this data set so I am not concerned with efficiency of sending resources over vast areas, I want to create a model that identifies the most blue tarps. p = 0.1 identified the most blue tarps, and had the highest precision when using 10-fold cross validation.

$$ClassBinary = Blue + GBSqr + RBSqr$$

### Logistic Regression Performance
Below I use the entire training data set on the model with p = 0.95 to get the training Accuracy, TPR, FPR, and Precision:

```{r logisticTrainingPerf, warning=FALSE}
glm.fits.model3 = glm(ClassBinary ~ Blue+Green+Red+GBSqr+RBSqr, binomial, data = haitiBinarySqrs)

 # fit the complex model
      glm.pred.model3 = glm.fits.model3 %>%  augment() %>% 
        dplyr::select(ClassBinary, .fitted)  %>% 
        mutate(ClassBinary=factor(ClassBinary))%>%
        mutate(.prediction=ifelse(1 - 1/(1 + exp(.fitted)) < 0.1, "No", "Yes")) %>%
        mutate(.prediction=fct_relevel(as_factor(.prediction), c("No", "Yes"))) 
      
caret::confusionMatrix(glm.pred.model3$.prediction, glm.pred.model3$ClassBinary)
...
<li>TPR: 1982 / 2022 = 0.9802</li>
<li>FPR: 231 / 61219 = 0.0038</li>
<li>Precision: 1989 / 2022 = 0.984</li>
</ul>

<br />
When using the entire training data set:
<ul>
<li>1,982 of the 2022 blue tarp pixels were identified by the model. Approximately, 2% of our blue tarp pixels will not be able to receive aid based on this model.</li>
<li>231 pixels were identified as blue tarps, but were not blue tarps. Approximately, one-third of 1% of blue tarps were identified as blue tarps and were not which will result un wasted resources. That seems small, but it represents 10% of the number of blue tarps, and thus, may have a 10% impact on distribution of resources.</li>
</ul>
...
preds = predict(glm.fits.model3, newdata=haitiBinarySqrs, type="response") # need to use type=response for probabilities.

##produce the numbers associated with classification table
rates = prediction(preds, haitiBinarySqrs$ClassBinary)
...
Subset selection using glmnet/ElasticNet provides an opportunity to see if additional predictors are of use improving the precision of our model.

I am going to add some additional interaction terms to the model. The interactions between Red, Blue, and Green are intuitively significant because the combinations of these values create the visible spectrum of color. 
...
I do not need to scale the predictors because the glmnet library will scale to standard deviation units.
...
y.haitiFull.train = haitiBinaryFull$ClassBinary
...
# TO DO: ADD A LOOP FOR alpha: 0 (ridge), 0.5 (elasticnet), and 1 (lasso)

#-- Get lambda sequence so consistent over all folds
lam_seq = glmnet(X.train, Y.train, family="binomial", alpha=0)$lambda

# TO DO: ADD LOOP OVER p values of .5, .6, .7, .75 .8, .85 .9, .95, 1.0
...
for(k in unique(folds)){
  ...
  #- fit ridge model(s) over all lambdas in lam_seq
  ridge = fit.ridge = glmnet(X.train[ind.train,], Y.train[ind.train], 
                             alpha = 0,          # this uses ridge penalty
                             family="binomial",  # this makes it logistic regression
                             lambda = lam_seq)   # all models use same lambda sequence
  
  #- make predictions on test set fold k
  pred = predict(ridge, X.train[ind.test, ], s=lam_seq, type = "response")
  
  #- evaluate fold k
  y_true = Y.train[ind.test]
  thres = .25
  y_hat = pred > thres
...
<tr>
  ...
  <td><!-- Log Reg AUROC -->0.9996</td>
  <td><!-- Log Reg Threshold -->0.1</td>
  <td><!-- Log Reg Accuracy -->0.9957</td>
  <td><!-- Log Reg TPR -->0.9802</td>
  <td><!-- Log Reg FPR -->0.0038</td>
  <td><!-- Log Reg Precision -->0.984</td>
</tr>
...
Only 3% of the observations in the training data are labeled "Blue Tarp". This is a very unbalanced training set. This is not unexpected because it would be extremely surprising if a high percentage of land area was covered by blue tarps. In fact, if that were the case, then I would expect it would not be challenging for aid workers to find survivors because survivors would be covering a high percentage of Haiti, or Haiti would just have blue tarps everywhere and they would be of little predictive power for finding survivors. That said, this imbalance of the data can give a false sense of faith in a model if only evaluating specificity, aka True Negative Rate of nonblue tarps. In this toy problem (as stated on Conclusion \#1) **a random guess of "not blue tarp" will be correct 97% of the time**, thus our model must perform better than a random guess, i.e. the Negative Predictive Value (NPV). In this problem the NPV could be thought of as a measure of efficiency of resource usage, i.e. are the predictions good at keeping our imagined aid workers from traveling to non blue tarps.<br /><br/>The NPV's for the models are:
<ul>
<li>Logistic Regression with Complex Model: 0.9993</li>
<li>LDA with Complex Model: 0.9949</li>
<li>QDA with Complex Model: 0.9969</li>
<li>KNN | k = 27 with Complex Model: 0.999</li>
<li>Penalized Log Reg with Complex Model: **TBD**</li>
</ul>
<br />
...
<ul>
<li>Logistic Regression with Complex Model: 0.0198</li>
...
<br />
I recommend working with experienced aid workers and local Haitian residents with knowledge of the land to determine the choice between, and balance of these considerations. If we want the model that is least likely to send aid workers to non blue tarps we will select **CHOOSE THE MODEL**; however if we want the model that correctly identified the greatest percentage of blue tarps we will select **CHOOSE THE MODEL**.
