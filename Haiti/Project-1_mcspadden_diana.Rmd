...
Below is a map of the **True Positives**, these are all locations in Carrefour.
```{r mapping-1}

locations_true_pos_sf = st_as_sf(true_pos_preds, coords = c("lon", "lat"), crs = 4326)
dim(locations_true_pos_sf)
mapview(locations_true_pos_sf)
```

Below are the **False Negatives**. An encouraging outcome of the mapping is that it is clear that the False Negatives were pixels that were subsets of a pixels for locations where True Positives were correctly identified. I guess that the false negatives may be pixels from the edges of blue tarps. I am very pleased that the 89 false negatives in the holdout data would not have kept resources from reaching survivors.

```{r mapping-2}

locations_false_neg_sf = st_as_sf(false_negative_preds, coords = c("lon", "lat"), crs = 4326)
dim(locations_false_neg_sf)
mapview(locations_false_neg_sf)
```

# Final Conclusions

<!--
Report on at least 6 conclusions. Give sufficient explanation and justification for each conclusion.

Four of your conclusions must be the following:
