...
<ul>
<li>Accuracy: 0.9945</li>
<li>TPR: 1738 / (1738 + 284): 0.859</li>
<li>FPR: 66 / (61153 + 66): 0.001</li>
<li>Precision: 0.9954</li>
</ul>
...
<ul>
<li>Accuracy: 0.9949</li>
<li>TPR: 1768 / (1768 + 254): 0.874</li>
<li>FPR: 70 / (61149 + 70): 0.001</li>
<li>Precision: 0.9959</li>
</ul>
...
After testing over ranges from 1 to 51, the best k was **27**. To speed processing of this document, I include 26, 27, and 28 as values for k.

```{r knnTry1, cache=TRUE, warning=FALSE}
set.seed(1976)
...
```{r rfROC, cache=TRUE, fig.width=10}
...
for (cost_val in c(1))
{
  for(i in 1:10)
  {
      ...
      for (thres in c(.2))
      {
          y_hat = attr(pred, "probabilities")[,"1"] > thres
          
          acc = sum(y_hat == (testData$ClassBinary == "1")) / nrow(testData)
          
          out_svm = bind_rows(out_svm, 
                tibble(thres = thres, cost = cost_val, pred_value = y_hat, truth_value = testData$ClassBinary, accuracy = acc, noProb = attr(pred, "probabilities")[,"0"], yesProb = attr(pred, "probabilities")[,"1"]))
      }
  }
}
...
```{r sv-ConfMatrix}
best_svm = out_svm %>% filter(thres == 0.2 & cost == 1.00) %>% mutate(truth_value = (truth_value == "1")) %>% mutate(pred_value = factor(pred_value), truth_value = factor(truth_value))
head(best_svm)
...
<li>AUC: 0.9995</li>
<li>TPR: 1945 / 2022 = 0.962</li>
<li>FPR: 155 / 61219 = 0.0025</li>
<li>Precision: 1945 / 2100 = 0.9262</li>
<li>Neg Pred Value: 61064 / 61141 = 0.999</li>
<li>False Pred Rate: 77 / 2022 = 0.038</li>
</ul>
...
<tr>
  <td>LDA</td>
  <td><!-- LDA Tuning --> N/A</td>
  <td><!-- LDA AUROC --> 0.9945<td>
  <td><!-- LDA Threshold -->0.3</td>
  <td><!-- LDA Accuracy -->0.9945</td>
  <td><!-- LDA TPR -->0.859</td>
  <td><!-- LDA FPR -->0.001</td>
  <td><!-- LDA Precision -->0.9954</td>
</tr>
...
<tr>
  ...
  <td><!-- QDA Threshold -->0.8</td>
  <td><!-- QDA Accuracy -->0.9949</td>
  <td><!-- QDA TPR -->0.874</td>
  <td><!-- QDA FPR -->0.001</td>
  <td><!-- QDA Precision -->0.9959</td>
</tr>
...
haitiAllTraining = haitiBinaryRF %>%
  mutate(ClassBinary01 = ifelse(ClassBinary == "No","0","1"))
...
```{r summarizeHoldOutBinaryByCategory}
...
```{r holdout-fitLogisticREgression}
# train the selected logistic regression model with the complete training data set
glm.all.train = glm(ClassBinary ~ Blue+Green+Red+GBSqr+RBSqr, binomial, data = haitiAllTraining)
```
...
```{r holdout-testLogisticRegression-ROC}
...
lda.cv.model.selected = train(ClassBinary ~ Blue+Green+Red+GBSqr+RBSqr, data = haitiAllTraining, method = "lda", trControl=trctrl.lda.selected, tuneLength = 10)
...
<ul>
<li>Accuracy: 0.9976</li>
<li>TPR: 12399 / (14480) = 0.856</li>
<li>FPR: 2727 / (1989697) = 0.001</li>
<li>Precision: 14391 / (38033) = 0.8197</li>
</ul>
...
qda.cv.model.selected = train(ClassBinary ~ Blue+Green+Red+GBSqr+RBSqr, data = haitiAllTraining, method = "qda", trControl=trctrl.qda.selected, tuneLength = 10)
...
<ul>
<li>Accuracy: 0.9906</li>
<li>TPR: 5753 / (14480) = 0.397</li>
<li>FPR: 10088 / (1989697) = 0.005</li>
<li>Precision: 14391 / (38033) = 0.3632</li>
</ul>
...
# fit the model on the entire training set
train_knn <- train(ClassBinary ~ Blue+Green+Red+GBSqr+RBSqr, data = haitiAllTraining, method = "knn", tuneGrid = data.frame(k=27))
...
<ul>
<li>Accuracy: 0.9921</li>
<li>TPR: 12622 / (14480) = 0.872</li>
<li>FPR: 14010 / (1989697) = 0.007</li>
<li>Precision: 12622 / (26632) = 0.4739</li>
</ul>
...
<li>Precision: 14380 / (29696) = 0.484</li>
</ul>
...
#- fit the random forest with all the training data and teh best lambda
# training
rf.training = randomForest(ClassBinary~Red+Blue+Green+GBSqr+RBSqr+RGSqr+RGBSqr+RG+RB+GB, data = haitiAllTraining, mtry=4, ntree=1000)
# test
rf.ho.preds = predict(rf.training, newdata = haiti_ho_full, type="prob")

rf.ho.preds[1:5,]


#- evaluate fold
y_true = haiti_ho_full %>% mutate(ClassBinary = (ClassBinary == "0"), Class0or1 = ifelse(ClassBinary == FALSE,0,1))
y_true_val = y_true$ClassBinary


# use tuned threshold
y_hat = rf.ho.preds[,2] > 0.6
...
#- output
out_yhat_ytrue = bind_rows(out_yhat_ytrue, 
          tibble(truth0or1 = y_true$Class0or1, X0 = rf.ho.preds[,1], X1 = rf.ho.preds[,2]))

all_out_yhat_ytrue = bind_rows(all_out_yhat_ytrue,
          tibble(true = y_true_val, hat = y_hat))


all_out_yhat_ytrue = all_out_yhat_ytrue %>% mutate(true = factor(true), hat = factor(hat))

#fct_count(all_out_yhat_ytrue$hat)
#fct_count(all_out_yhat_ytrue$true)

caret::confusionMatrix(all_out_yhat_ytrue$hat, all_out_yhat_ytrue$true)
...
<ul>
<li>Accuracy: 0.9934</li>
<li>TPR: 8232 / (14480) = 0.569</li>
<li>FPR: 6956 / (1989697) = 0.003</li>
<li>Precision: 0.5420</li>
</ul>
...
function_ROC_AUC(out_yhat_ytrue$X1, out_yhat_ytrue$truth0or1, "ROC Hold-out Random Forest")
...
<tr>
  ...
  <td><!-- Log Reg Threshold -->0.1 (calculated from training data)</td>
...
<tr>
  ...
  <td><!-- LDA Threshold -->0.3</td>
  <td><!-- LDA Accuracy -->0.9976</td>
  <td><!-- LDA TPR -->0.856</td>
  <td><!-- LDA FPR -->0.001</td>
  <td><!-- LDA Precision -->0.8197</td>
</tr>
...
<tr>
  ...
  <td><!-- QDA Threshold -->0.8</td>
  <td><!-- QDA Accuracy -->0.9906</td>
  <td><!-- QDA TPR -->0.397</td>
  <td><!-- QDA FPR -->0.005</td>
  <td><!-- QDA Precision -->0.3632</td>
</tr>
...
<tr>
  <td>KNN</td>
  <td><!-- KNN Tuning --> k = 27</td>
  <td><!-- KNN AUROC -->0.954</td>
  <td><!-- KNN Threshold -->0.5</td>
  <td><!-- KNN Accuracy -->0.9921</td>
  <td><!-- KNN TPR -->0.872</td>
  <td><!-- KNN FPR -->0.007</td>
  <td><!-- KNN Precision -->0.4739</td>
</tr>
...
<tr>
  ...
  <td><!-- Random Forest Threshold -->0.6</td>
  <td><!-- Random Forest Accuracy -->0.9934</td>
  <td><!-- Random Forest TPR -->0.569</td>
  <td><!-- Random Forest FPR -->0.003</td>
  <td><!-- Random Forest Precision -->0.542</td>
</tr>
