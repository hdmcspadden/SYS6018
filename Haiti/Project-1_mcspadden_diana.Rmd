...
I tested with thresholds in c(.1, .2, .3, .4, .5, .6, .7, .8, .85, .9, .95). The best performing threshold on accuracy was 0.3. For illustrative purposes I run with 0.2, 0.3, and 0.4.
...
roc_pred = ROCR::prediction(out_all_preds$Yes, out_best_lda$truth)
...
<li>Precision: 14391 / (38033) = 0.38</li>
</ul>

```{r holdout-testLogisticRegression-ROC}
##produce the numbers associated with classification table
rates = prediction(glm.preds$.fitted, glm.preds$ClassBinary)

##store the true positive and false postive rates
roc_result = performance(rates, measure="tpr", x.measure="fpr")

##plot ROC curve and overlay the diagonal line for random guessing
plot(roc_result, main="ROC Curve Hold-out Logistic Regression")
lines(x = c(0,1), y = c(0,1), col="red")
