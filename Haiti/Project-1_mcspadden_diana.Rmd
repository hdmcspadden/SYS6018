...
![RF Tuning Results](RFTuning.JPG)


![RF MTRY Results](RF_MTRY_RESULT.JPG)
...
```{r rfThresholdTPRResults, cache=TRUE}
#head(out_yhat_ytrue)
...
```{r rfThresholdAccuracyResults, cache=TRUE}
#head(out_rf)
...
<ul>
<li>Accuracy: 0.9954</li>
<li>AUC: 0.994</li>
<li>TPR: 1992 / 2022 = 0.985</li>
<li>FPR: 261 / 61219 = 0.004</li>
<li>Precision: 1949 / 2129 = 0.884</li>
</ul>
...
<ul>
<li>Accuracy: 0.9962</li>
<li>AUC: 0.9995</li>
<li>TPR: 1972 / 2022 = 0.975</li>
<li>FPR: 190 / 61219 = 0.003</li>
<li>Precision: 0.9121</li>
</ul>
...
<tr>
  ...
  <td><!-- Random Forest Accuracy -->0.9954</td>
  <td><!-- Random Forest TPR -->0.985</td>
  <td><!-- Random Forest FPR -->0.004</td>
  <td><!-- Random Forest Precision -->0.884</td>
</tr>
...
<tr>
  ...
  <td><!-- SVM Accuracy -->0.9962</td>
  <td><!-- SVM TPR -->0.975</td>
  <td><!-- SVM FPR -->0.003</td>
  <td><!-- SVM Precision -->0.9121</td>
</tr>
</table>


#  PART ONE: Training Conclusions 
...
```{r colorValuesBinarySqrsCatsHoldout, cache=TRUE, fig.height=5, fig.width=5}
library(ggplot2)

ggplot(haiti_ho_full, aes(x=ClassBinary, y=Red)) + geom_boxplot()

redplotB = ggplot(haiti_ho_full, aes(x=ClassBinary, y=RBSqr)) + geom_boxplot(col='red')

greenplotB = ggplot(haiti_ho_full, aes(x=ClassBinary, y=GBSqr)) + geom_boxplot(col='darkgreen')

blueplotB = ggplot(haiti_ho_full, aes(x=ClassBinary, y=Blue)) + geom_boxplot(col='darkblue')
...
<ul>
<li>Accuracy: 0.9827</li>
<li>TPR: 13100 / (14480) = 0.905</li>
<li>FPR: 33313 / (1989697) = 0.017</li>
<li>Precision: 0.2822</li>
</ul>
...
<li>TPR: 14380 / (14480) = 0.993</li>
<li>FPR: 15316 / (1989697) = 0.008</li>
<li>Precision: 0.4842</li>
</ul>
...
<ul>
<li>Accuracy: 0.9816</li>
<li>TPR: 14393 / (14480) = 0.994</li>
<li>FPR: 36821 / (1989697) = 0.018</li>
...
<tr>
  ...
  <td><!-- KNN Threshold -->0.1</td>
  <td><!-- KNN Accuracy -->0.9827</td>
  <td><!-- KNN TPR -->0.905</td>
  <td><!-- KNN FPR -->0.017</td>
  <td><!-- KNN Precision -->0.2822</td>
</tr>
...
<tr>
  ...
  <td><!-- Penalized Log Reg Tuning -->alpha = 1, lamda = 5.630136e-05</td>
  <td><!-- Penalized Log Reg AUROC -->0.997</td>
  <td><!-- Penalized Log Reg Threshold -->0.2</td>
  <td><!-- Penalized Log Reg Accuracy -->0.9923</td>
  <td><!-- Penalized Log Reg TPR -->0.993</td>
  <td><!-- Penalized Log Reg FPR -->0.008</td>
  <td><!-- Penalized Log Reg Precision -->0.4842</td>
</tr>
...
<tr>
  ...
  <td><!-- SVM AUROC -->0.9995</td>
  <td><!-- SVM Threshold -->0.1</td>
  <td><!-- SVM Accuracy -->0.9816</td>
  <td><!-- SVM TPR -->**0.994**</td>
  <td><!-- SVM FPR -->0.018</td>
  <td><!-- SVM Precision -->0.2810</td>
</tr>
</table>
...
#### KKN Model
The KNN model was tuned for threshold and number of neighbors using true positive rate. The model used the additive model with the following features:
<ul>
<li>Blue</li>
<li>Green</li>
<li>Red</li>
...
The KNN model was tuned for number of neighbors using true positive rate to number of neighbors == 27. The best threshold, by true positive rate, was 0.1.

#### Random Forest

Features were added to the random forest because of a random forest ability to identify and used the following features:
<ul>
<li>**Red**</li>
<li>**Blue**</li>
<li>**Green**</li>
...
The random forest model was tuned using true positive rate to use 4 parameters at each split and 1000 trees. The best threshold, by true positive rate, was 0.1.

#### Logistic Regression

The logistic regression model used an additive model of Blue, Green, Red, (Green + Blue)^2, and (Red + Blue)^2. The best threshold, by true positive rate, was 0.1.

#### Comments On The Three Cross-Validation Best Performing Models

All three of the best performing models use a runed threshold of 0.1; however, 0.1 was the lowests threshold evaluated for best true positive rate. A lower threshold may have performed better by true positive rate. The downside of a lowering the threshold is that there will be more false positives identified.

The logistic regression model had a **much lower** training time as cpompared to the other two best performing models. Both the KNN and random forest models took significant time to train with only ~60,000 observations. The 60,000 observations only included locations in a small area of haiti, so larger search areas would result in much longer training time.

### Hold-out Data Results

Using training-data-tuned parameters, two models performed best on the hold-out data evaluated on true positive rate:

1. Logistic Regression Model correctly classified 99.4% of the blue tarps.
2. The Support Vector Machine correctly classified 99.4% of the blue tarps.

Both the logistic regression and the SVM correctly classified 99.4% of the holdout data; however the logistic regression model classified fewer false positives (1.2% vs. 1.8%) which would result in fewer wasted resources.

While the logistic regression and SVM models performed better on true positive rate on the hold-out data as compared to the training data, they performed significantly worse on precision when comparing the hold-out data to training data (0.38 and 0.28 respectively). None of the models had precision over 0.50 using the hold-out data indicating that there were many false positives identified in the hold-out data. The tuned threshold would need to be tuned differently on the training data if there were limited volunteers and little time to find survivors, which would mean that false positives would result in wasted time and resources.
...
My training and hold-out results are compatible. The logistic regression model performed in the top three models based on TPR on the training data, and was second (tied) for AUC. Based on visual inspection of the training data the data could be linearly separated, and the good performance of the logistic regression model makes sense.

It also makes sense that there were differences in performance between the training and hold-out data because

1. the hold-out data was different
2. the hold-out data had proportionally even fewer blue tarps (1% compared to 3%). Predicting rare occurances is challenging, but both on training and test data we were able to classify blue tarps.
...
The metric in the results tables for both training and hold-out data sets are:
<ul>
<li>**AUROC** (area under the Receover Operator Characteristic curve)
<br />
provides a measure of how well, i.e. how effectively, out model turns posterior probabilities into true classifications. An ROC curve is plotted by sensistivity (true positive rate) by (1 - specificity) (false positive rate). The AUC values allow us to compare one model to another: the greater the AUC, the better. Using the training data KNN, lasso, and logistic regression performed best on AUC. Lasso and logistic regression performed equally well: 0.9996, while KNN performed slightly better with 0.9997 AUC.
</li>
<li>**Accuracy**
<br />
provides a measure of total correct predictions over the total number of observations. This measure is not of as much utility in the haiti example for a few reasons: 1) the unbalanced data set. There are few blue tarps in the training data set (3%) and proportionally evern fewer in the hold-out data set (1%). A random guess of "not blue tarp" would be "accruate" 97% of the time in the training data, and 99% of the time for the hold-out data. 

Also, because I have made the decision to not particularly care whether I correctly identify non-blue tarps, and because non-blue tarps make up the vast majority of my data, accuracy is not the best metric to use to tune or evaluate the models in this toy problem. Although of less utility, this metric was used to tune some of the tuning parameters from the training models.

It is of interest that using the hold-out/test data none of the models performed better than 99% accuracy, which would be the best odds guess. However, we aren't interested in accuracy, but in true positive rate ...
</li>
<li>**TPR** (true positive rate)
<br />
is the metric of most utility in this toy problem as this identifies which model predicts the highest percentage of blue tarps. THis metric was used for tuning thresholds and some tuning parameters.
</li>
<li>**FPR** (false positive rate)
<br />
measure how many non-blue tarps were predicted to be blue tarps. If resources, volunteers, or time is severly limited, this metric should be examined and used for the Haiti problem.
</li>
<li>**Precision**
<br />
provides a similar/mirror measure to false positive rate when examining the performance of our Haiti models. Precision provides a metric of true positives over predicted condition positive. In other words of all the "yes" blue tarp predictions, what proportion are truely blue tarps. Like FPR, this provides an idea of resource efficiency.
</li>
</ul>

### Conclusion \#5: Does The Evidence Warrant Concern About False Negatives?
<!-- DONE -->
#### A Map View of True Positives and False Negatives
...
``` {r mapping1, cache=TRUE}
glm.preds = glm.preds %>% mutate(lat = Lat, lng = Lon)

#head(glm.preds)

true_pos_or_false_negs_preds = glm.preds %>% 
  filter((ClassBinary == 1 & .prediction == 1) | (ClassBinary == 1 & .prediction == 0))
...
```{r mapLeafLetTruePosAndFalseNegatives, cache=TRUE}
# Create a palette that maps factor levels to colors
pal <- colorFactor(c("red", "blue"), domain = c("0", "1"))

leaflet(true_pos_or_false_negs_preds) %>%
  addTiles() %>%
  addCircleMarkers(lng = ~lng, lat = ~lat, color = ~pal(.prediction))
```
The leaflet library maps do not allow for zooming in at the individual pixel level of the dataset; however, the mapview library does. However, I was unable to get the mapview library to knit to html, so I include the code below and screen shots from example "blue tarps" in the dataframe containing labeled "True Positive" and "False Negative" results from the holdout data set. From this visualization you can see the that the false negatives are all from tarps where the majority of pixels were correctly predicted as true positives.


```{r mapViewVersion, eval = FALSE, cache=TRUE}

true_pos_or_false_negs_preds_sf = st_as_sf(true_pos_or_false_negs_preds, coords = c("lon", "lat"), crs = 4326)

mapview(true_pos_or_false_negs_preds_sf, zcol = ".prediction", burst = TRUE)
```
#### Using mapview Library:
![All True Positive (yellow) and False Negative (purple) predictions](MapViewExampleAll.JPG)


![Example Tarp 1: True Positives (yellow) and False Negative (purple)](FalseNegativeExample1.JPG)


![Example Tarp 2: True Positives (yellow) and False Negative (purple)](FalseNegativeExample2.JPG)


![Example Tarp 3: True Positives (yellow) and False Negative (purple)](FalseNegativeExample3.JPG)

### Conclusion \#6: Another Reason to Use Logistic Regression
<!-- DONE -->
<!-- TODO A discussion of processing speed. -->

KNN, random forest, SVM, and lasso models performed better than the LDA, QDA. However, KNN, random forest, SVM, lasso models took significantly more time to train than the simplier logistic regression, LDA and QDA models. I would imagine that if rescue efforts continue for many days the model would be periodically re-trained with new labeled/validated data. If time is of the essence, which in this scenario it would be because getting predictions for locations of survivors is only the first step in making a plan for volunteers to reach the survivors, it may make sense to select a model that does not take as long to train. Luckily, the data was linearly separable and the logistic regression model performed well, and took little time to train.
...
<li>ROC and AUC Explained: <https://statquest.org/roc-and-auc-clearly-explained/></li>
</ul>
