...
# raw == TRUE gives a difference basis for estimate
...
# another way to do it with I function - this is the same as raw = TRUE
fit2a <- lm(wage~age+I(age^2)+I(age^3)+I(age^4), data=ISLR::Wage)
fit2a %>% broom::tidy()

#fit2b=lm(wage~cbind(age,age^2,age^3,age^4),data=Wage)

# with cbind() command combines all the variables as columns in the data set
...
# want to plot the standard error bounds
# want to plot from the min and max age limits.

# orthog polynomials of age
...
# add the poly columns to the wage tibble
wage[,names(X)] <- X
...
```{r}
# get the first 30 values of age????
...
# plot the upper and lower bounds of the standard errror
...
# notice that se=TRUE, alpha=1, poly formula with degree 4
# this is the left side plot below
...
## ANOVA F stat says the 5th degree on is insignificant
First three are not good. But the 4th is good.
```{r}
...
# try to determine which poly is the appropriate one
...
```
**Cut()** function: Sometimes it is useful to categorize the values of a continuous variable in different levels of a factor. For that purpose, you can use the R cut function. 


Fit a binomial for wage > 250.

se = TRUE is predict using standard error

preds@fit gives the linear prob for the odds.
```{r}
...
# matlines are standard error bands
#matlines(age.grid,se.bands,lwd=1,col="blue",lty=3)
...
# lo = local regression with span 0.7
