...
```
**prediction interval**
For lstat = 5, lstat = 10, or lstat=15
```{r}
...
plot(hatvalues(lm.fit)) # high leverage points
```
...
Measure multicolinearity with all other variables
1 is minimum value
greater than 5 is considered high.

```{r}
library(car)
vif(lm.fit.all)
```
**rad and tax seem to have multicolinearity**
...
```
#### Interaction Terms
make linear model with an interaction terms with medv and lstat
...
summary(lm.fit_higherOrderTerms)
```
