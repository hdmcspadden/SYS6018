...
Drop NA's from hitters - you can see the NA's in the salary column with summary
```{r hitters}
#library(ISLR)
#fix(Hitters)
names(Hitters)
...
Use regsubsets to determine best model. RegSubSets reviews all possible models of each size and returns best for each number of predictors.
...
### Model Search
```{r search, fig.width=12, fig.height=8}
...
# arrange() sorts in ASC order by the variable
...
library(purrr)
# set_names: when it prints it out these are the names
...
```{r myOwnCode1}
print(ridgeCV$bestTune$lambda)

summary(ridgeCV$finalModel)

print(ridgeCV$finalModel$lambdaOpt)
```

In ridge regression we still have all the predictors. That's why length is still 20.
...
```
## My own code
Try the glmnet with alpha = 1 for lasso
...
lassoCV <- caret::train(Salary~., data=hitters_drop_na[train_indices,],
             ...
             tuneGrid=data.frame(alpha=rep(1,100), 
                                 lambda=10^seq(2.4,0,length=100)),
             trControl=trainControl("cv", number=5, returnResamp='all', savePredictions = "all"))

# look at the results of the lasso
lassoCV$results
```

```{r myOwnCode1}
print(lassoCV$bestTune$lambda)

summary(lassoCV$finalModel)

print(lassoCV$finalModel$lambdaOpt)

print(coef(lassoCV$finalModel, lassoCV$bestTune$lambda) %>% length())

```


## Back to Lab Code with Ridge Version
```{r}
#ridge.mod$lambda[50]
...
Is this showing that as lambda increases the change in residual mean squared error???

Also plots each of the 5 folds RMSE.

arrange() orders the rows of a data frame by the values of selected columns. So this is grouping by lambda for each of the k folds.

rowid_to_column(), which adds a column at the start of the dataframe of ascending sequential row ids starting at 1.
```{r ridge_cv}
...
hitters_drop_na_regsubsetsNaming <- hitters_drop_na
names(hitters_drop_na_regsubsetsNaming)[[14]] <- 'LeagueN'
names(hitters_drop_na_regsubsetsNaming)[[15]] <- 'DivisionW'predict
names(hitters_drop_na_regsubsetsNaming)[[20]] <- 'NewLeagueN'
